    ...
    if len(countries)>18:
        ...
        with average_map:
            ...
            html_string = (
                ...
                '<b>Figure 4a: Average network footprint for the selected '
                'time-period.</b>'
                '</p>'
            )
            display(HTML(html_string))
            file_path = os.path.join(output_network_path, 'average_map.png')
            relative_file_path = os.path.join(
                relative_network_path, 'average_map.png'
            )
        ...
        with landcover_view:
            network_analysis_functions.landcover_view(
                nwc,
                countries,
                pngfile='landcover_view.png',
                output=output_network_path
            )
            html_string = (
                '<p style="font-size:15px;">'
                '<b>Figure 4b: Land cover shares within the average network '
                'footprint compared to country shares of land cover for '
                'selected countries.</b>'
                '</p>'
            )
            display(HTML(html_string))
            file_path = os.path.join(output_network_path, 'landcover_view.png')
            relative_file_path = os.path.join(
                relative_network_path, 'landcover_view.png'
            )
            if os.path.exists(file_path):
                html_string = (
                    '<p style="font-size:18px">'
                    '<br>Access graph <a href='
                    f'{relative_file_path} target="_blank">here'
                    f'</a><br><br>'
                )
                display(HTML(html_string))
        ...
        with flux_view:
            success = network_analysis_functions.flux_breakdown_countries_percentages(
                nwc,
                countries,
                pngfile='flux_view.png',
                output=output_network_path
            )
            ...
            if success:
                file_path = os.path.join(output_network_path,
                                         'flux_view.png')
                relative_file_path = os.path.join(
                    relative_network_path, 'flux_view.png'
                )
                if os.path.exists(file_path):
                    html_string = (
                        '<p style="font-size:18px">'
                ...
                with representation:
                    network_analysis_functions.share_representaiton_table(
                        nwc,
                        countries,
                        csvfile='representation.csv',
                        output=output_network_path
                    )
                    html_string = (
                        '<p style="font-size:15px;">'
                        'Data used to create <b>Figures 5b and 8b: Network '
                        'representation of land cover associated fluxes ('
                        f'{nwc["component"].upper()}). Representation is '
                        f'established by comparing the "network views" (the '
                        f'network footprints) of the fluxes to "equal views"'
                        f' (sensing capacity evenly distributed) of the '
                        f'fluxes within selected countries for the time-steps'
                        f' in the selected date range.</b>'
                        f'</p>'
                    )
                    display(HTML(html_string))
                    file_path = os.path.join(output_network_path,
                                             'representation.csv')
                    relative_file_path = os.path.join(
                        relative_network_path, 'representation.csv'
                    )
                    if os.path.exists(file_path):
                        html_string = (
                            '<p style="font-size:18px">'
                            '<br>Access as csv-file <a href='
                            f'{relative_file_path} target="_blank">here'
                            f'</a><br><br>'
                        )
                        display(HTML(html_string))
