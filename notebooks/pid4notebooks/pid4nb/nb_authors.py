...
def author_wdgt_form():
    ...
    with author_description:
        printmd("<p>Fill the fields below with information related to the notebook's authors. Keep in mind that you have to provide information for at least one author. Fields marked with (*) are mandatory. </p><br>")
...
def to_dict(creators_wdgt_form):
    ...
    def get_creator_info(creator_obj):
        
        #Store orcid-value:
        orcid = creator_obj.children[0].children[0].value
        ...
        if(orcid == ''):
            ...
            #Check if ORCiD is provided as URL:
            if('https://orcid.org/' in orcid):
                orcid = orcid.replace('https://orcid.org/', '')
                                
        return {'nameType' : 'Personal',
                'creatorName' : creator_obj.children[2].value +', '+ creator_obj.children[1].value,
                'schemeURI' : schemeURI,
                'nameIdentifierScheme' : nameIdentifierScheme,
                'nameIdentifier' : orcid,
                'givenName' : creator_obj.children[1].value,
                'familyName' : creator_obj.children[2].value,
                'affiliation' : creator_obj.children[3].value,
                'email' : creator_obj.children[4].value}
    
    
    #Return metadata dict for authors:    
    return {"creators" : [get_creator_info(creator)
