...
#            sys.exit(1)
        ...
        form_items = [
            Box([self.Input, self.moguntia, self.fetch ,self.wdoit , self.woplot], layout = form_item_layout),
            ...
            Box([Label(value='yMin, yMax'), self.wmin, self.wmax],  layout=form_item_layout),
            Box([Label(value='xMin, xMax'), self.wtmin, self.wtmax],  layout=form_item_layout)
            ]
...
# start the main interaction window:
    ...
    def run_moguntia(self, infile):
        ...
        with open(os.path.join(self.localpath, infile), 'r') as xfile:
            outp = subprocess.check_output([self.moguntiapathe + '/MODEL_ext/MOGUNTIA'], stdin=xfile)
    ...
    def Fetch(self,dummy):
        ...
        xfile.close()
        xfile = open(os.path.join(self.localpath,'OUTPUT',self.title+'files_written'))
        lines = xfile.readlines()
        lines.sort()
        ...
        for ioutput in options:
            if (ioutput.endswith('stations')):
                ...
                self.stations = True
                ofile = open(os.path.join(self.localpath,'OUTPUT',ioutput),'r')
...
# also create the overplot window:
    ...
    def plot_za(self,ioutput):
        ...
        with open(os.path.join(self.localpath,'OUTPUT',ioutput),'r') as ofile:
            lines = ofile.readlines()
