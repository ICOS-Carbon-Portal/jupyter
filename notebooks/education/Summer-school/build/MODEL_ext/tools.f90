...
integer ::i,j
character*80 :: line
real :: scalingsfactor, s1, s2, s3

sink = 0.0
sinke = 0.0
if (sink_flag) then
   !WP! if (abs(co2_land_sink(1)) > tiny(co2_land_sink)) then
   read(unit_sinke,*) line
   !print *,line
   read(unit_sinke,*) sinke   ! land sink in mol/m2/s
   !WP! sinke = sinke*co2_land_sink(1)   ! convert to right sink strength
   !WP! endif
   read(unit_sink,*) line
   !print *,line
   ...
   !s2 = 0.0
   !s3 = 0.0
   !call area(nu,nv,dxyp)
   do j=1,nv
   do i=1,nu
      select case(j)
      case(1:6)
              scalingsfactor = co2_land_sink(1)
              !s1 = s1 + sinke(i,j)*dxyp(j)
      case(7:12)
              scalingsfactor = co2_land_sink(2)
              !s2 = s2 + sinke(i,j)*dxyp(j)
      case(13:18)
              scalingsfactor = co2_land_sink(3)
              !s3 = s3 + sinke(i,j)*dxyp(j)
      end select
      sink(i,j,1) = dtim*(sink(i,j,1)+scalingsfactor*sinke(i,j))*avo/(air(i,j,1)*dthick(i,j,1)*1e6)  !delta mixing-ratio/timestep
   enddo
   enddo
   print*, 'WP ',co2_land_sink
   !do j=1,nv
   !do i=1,nu
   !   scalingsfactor = co2_land_sink(3)
   !   if (j<12) scalingsfactor = co2_land_sink(2)
   !   if (j<6) scalingsfactor = co2_land_sink(1)
!
!      sink(i,j,1) = dtim*(sink(i,j,1)+scalingsfactor*sinke(i,j))*avo/(air(i,j,1)*dthick(i,j,1)*1e6)  !delta mixing-ratio/timestep
!   enddo
!   enddo
