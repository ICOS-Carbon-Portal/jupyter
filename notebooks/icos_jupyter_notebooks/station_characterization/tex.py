...
def _template_specifications(standalone=False):
    ...
    tex = """
        ...
        The same type of values that are presented in the seasonal variations table but for the average footprint of the selected data range are used in the \\textbf{multiple variable graph} to place the selected station relative to reference atmospheric stations in the ICOS network. The value on the y-axis is relative to the minimum and maximum values of these ICOS stations. 0\% indicates the station with the lowest variable value, and 100\% the highest. 
