...
def _template_specifications(standalone=False):
    ...
    tex = """
        ...
        All the visualizations and information in this document are based on footprints generated by \\href{http://stilt-model.org/index.php/Main/HomePage}{STILT}, a lagrangian atmospheric transport model, implemented as \href{https://www.icos-cp.eu/data-services/tools/stilt-footprint}{online tool} at the ICOS Carbon Portal. Output footprints are presented on a grid with 1/12×1/8 degrees cells (approximately 10km x 10km) where the cell values represent the cell area’s estimated surface influence (“sensitivity”) in ppm / (\\SI{}{\\micro\\mol}/ (m²s)) on the atmospheric tracer concentration at the station. The footprints for a given station vary in time according to meteorological conditions, in this model represented by 3-hourly operational ECMWF-IFS analysis/forecasts. Modelled concentration can be calculated by combining individual footprints with an anthropogenic emission inventory (\\href{https://doi.org/10.5194/essd-11-959-2019}{EDGAR}) and a biogenic model (\\href{https://doi.org/10.1029/2006GB002735}{VPRM}), which models the exchange of the tracer between the atmosphere and the natural surface vegetation.

        For all maps and the land cover polar graph, an average footprint for the specified date range has been used. In the \\textbf{sensitivity area map} the 192 000 cells have been aggregated depending on their distance to, and direction of, the station. A uniform flux of 1 \\SI{}{\\micro\\mol}/ (m²s) is assumed. The most important 50\% sensitivity is derived by including the sensitivity values of the footprint cells in descending order until 50\% of the total sensitivity is reached. See the \\href{https://exploredata.icos-cp.eu/user/user/notebooks/icos_jupyter_notebooks/visualization_average_footprints.ipynb}{visualize average footprints notebook} for more information. The same aggregation principles were used for the \\textbf{population sensitivity map} and \\textbf{point source contribution map}, where the average footprint cells were multiplied by underlying ancillary data layers. The unit of the maps is either percent or the absolute value of the aggregated cells. 100\% in terms of the sensitivity map is the value of all aggregated cells summarized. 100\% in terms of the population map is the value of all aggregated (total population count * sensitivity) cells summarized. The population data are from \\href{https://sedac.ciesin.columbia.edu/data/set/gpw-v4-population-count-rev11/data-download}{SEDAC} (2020) and point source emissions data from \\href{https://www.eea.europa.eu/data-and-maps/data/member-states-reporting-art-7-under-the-european-pollutant-release-and-transfer-register-e-prtr-regulation-22}{E-PRTR} (2017). The resulting values in the population map are mainly interesting for intercomparisons between areas, whereas the point source emissions were converted to estimated contribution to the {\\ensuremath{\\mathrm{CO_2}}} mole fraction at the station. The descriptions of the maps include what quartiles the station falls into in terms of absolute ranking among selected ICOS atmospheric stations. The first quartile includes the stations with the lowest total average values.
        

        The \\textbf{land cover polar graph} is like the maps in that the centre represents the location of the station and the direction bins have been used to aggregate data but do not represent the distance of the land cover type to the station. The area of a land cover type in the figure represents its presence in each direction of the station. The overall dominant land cover type is closest to the centre, with the less significant land cover types displayed in descending order. The \\textbf{land cover bar graph} is a simpler way to visualize land cover type by direction. Eight 45 degrees bins around the station are represented by stacked bars. The land cover data is from \\href{https://www.nature.com/articles/s41467-021-22702-2} {HILDA} (2018). The original land cover types are used except for the aggregation of classes to represent 'broad leaf forest' (HILDA codes 42 + 44), 'coniferous forest' (HILDA codes 41 + 43), 'mixed forest' (HILDA codes 40 + 45) and 'other' (HILDA codes 66 + 77).
        ...
        The same type of values that are presented in the seasonal variations table, but for the average footprint of the selected date range, is used in the \\textbf{multiple variables graph} to place the selected station relative to reference atmospheric stations in the ICOS network. The value on the y-axis is relative to the minimum and maximum values of these ICOS stations. 0\% indicates the station with the lowest variable value, and 100\% the highest. 
...
def _template():
    ...
    tex = """
        ...
        \\usepackage[bottom=2cm,top=2.1cm, left=1in, right=1in]{geometry}
        ...
        \\AddToShipoutPictureBG*{%
          \\AtPageUpperRight{\\raisebox{-\height}{\includegraphics[trim={0 0 10cm 0},clip, width=5cm]{**logo**}}}}

        \\begin{Large}
        \\begin{center}
        \\textbf{**name** station characterisation}
        \\end{center}
        \\end{Large}\n
        \\bigskip
...
the end of this document for further information and explanations.
        ...
        \\hspace{-0.6cm}%
        \\captionsetup{labelformat=empty}
        \\parbox{7.9cm}{\\caption{\\begin{small}The \\textbf{land cover polar graph} summarizes the distribution of land cover types in the average footprint around the station (located in the centre of the graph). Note that the area of a land cover type with the highest contribution is located closest to the centre. \\end{small}\includegraphics[trim={21.8cm 0 0 9cm},clip, width=0.22\\textwidth]{**landcover_windrose**}}}}
        \\end{figure}
...
def generate_pdf_tex(stc):
    ...
    area_50 = str("%.0f" % stc.settings['areaFifty']) 
    ...
    tex=tex.replace('**area_50**', area_50)
    ...
    if "_" in stc.settings['stationCode']:
        code_for_update = stc.settings['stationCode']
        code_for_update = code_for_update.replace("_", " ")
        tex=tex.replace('**station_code**', code_for_update)
    else:
    ...
    if stc.settings['stilt']['icos']:
        if stc.settings['icos']['siteType']=='mountain' or stc.settings['icos']['siteType']=='Mountain':
            tex=tex.replace('**mountain_message**', '(might be different from station intake height because it is a mountain station, see \\href{https://www.icos-cp.eu/about-stilt-calculator}{link})')
    ...
    if stc.settings['stilt']['icos']: 
        if '_' in stc.settings['icos']['name']:
            name_for_update = stc.settings['icos']['name']
            name_for_update = name_for_update.replace("_", "\_")
            tex=tex.replace('**name**', name_for_update)
        else:
        ...
        if '_' in stc.settings['stilt']['name']:
            ...
            name_for_update = name_for_update.replace("_", " ")
            tex=tex.replace('**name**', name_for_update)
        else:
