...
with file_info:
    ...
    display(HTML('<p style="font-size:15px;font-weight:bold;">Available network footprint folders: </p>'))
    for root, dirs, files in os.walk(r'' + path_network_footprints):
        for file in files:
            if file.endswith('.json'):
                ...
                stations_string = ', '.join([str(elem) for elem in network_footprint_info_load['stations']])[:-1] 
                
                display(HTML('<p style="font-size:15px;"><b>Foldername: </b> ' + str(network_footprint_info_load['fileName']) + '<br><b>Stations: </b> ' + stations_string + '<br><b>Year: </b> ' +  str(network_footprint_info_load['startYear']) + '<br><b>Threshold:</b> ' +  str(network_footprint_info_load['fpPercent']*100)+ '%</p>'))
                
    display(HTML('<p style="font-size:15px;font-weight:bold;"><br>Available local network footprint folders: </p>'))
    ...
    display(HTML('<p style="font-size:15px;">In the future it will be possible to produce a <mark>different footprint folder and associated representation files</mark>. For urgent requests, please contact ida.storm@nateko.lu.se.<br><br></p>'))
...
extended_network_choice = Dropdown(options = list_extended_network,
                   description = 'Extended network (optional)',
                   value='No extension',
                   disabled= False,
...
#Open form object:
with form_out:
    display(file_info, settings_grid_1,settings_grid_2,settings_grid_3, settings_grid_4, settings_grid_5, extended_network_choice, update_button, output_warning, header_output, average_map, landcover_view, flux_view, representation)
