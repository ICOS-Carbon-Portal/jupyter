...
class Translator():
    """
        Use an instance of Translator in order to converst ICOS variables 
        or ICOS units into LaTeX strings to prettify plots etc. 
    ...
    def __process_configs(self, **configs):
        ...
        known_keys = set(Translator.__layout.keys()).union(['debug'])
        ...
        unknown_keys = set(configs.keys()).difference(known_keys)
    ...
    """
        ...
        if key in ['debug', 'use_exp']:
            if isinstance(value,bool):
                ...
                value_error = True
        elif key in Translator.__layout.keys() and isinstance(Translator.__layout[key], dict):
